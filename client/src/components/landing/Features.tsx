...
export const Features: React.FC = () => {
  return (
    <section id="features-section" className="py-24 bg-white relative overflow-hidden scroll-mt-16">
      {/* Background Grid */}
      <div className="absolute inset-0 bg-grid-slate-100 [mask-image:linear-gradient(180deg,white,rgba(255,255,255,0))]" />

      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <FadeIn className="text-center mb-16">
          <span className="text-violet-600 font-semibold tracking-wide uppercase text-sm bg-violet-50 px-3 py-1 rounded-full border border-violet-100">Features</span>
          <h2 className="text-3xl md:text-5xl font-bold text-slate-900 mt-6 mb-6">Built for Your Wellbeing</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, idx) => (
            <FadeIn key={idx} delay={idx * 100} className="h-full">
              <div className="group h-full p-8 bg-white rounded-2xl border border-slate-200 hover:border-violet-200 shadow-sm hover:shadow-2xl hover:shadow-violet-100 transition-all duration-300 relative overflow-hidden">
                 <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-violet-50 to-transparent rounded-bl-full -mr-4 -mt-4 opacity-0 group-hover:opacity-100 transition-opacity" />

                <div className="w-14 h-14 bg-slate-50 rounded-xl flex items-center justify-center text-slate-600 mb-6 group-hover:scale-110 group-hover:bg-violet-600 group-hover:text-white transition-all duration-300 shadow-sm relative z-10">
