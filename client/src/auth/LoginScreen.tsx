...
import { Eye, EyeOff, Loader2, ArrowRight, Lock, BookLock, Shield, ArrowLeft } from 'lucide-react';
...
export const LoginScreen: React.FC = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await register({
          name: regName, email: regEmail, username: regUsername, password: regPassword, diaryPassword: regDiaryPassword,
  ...
  return (
    <div className="flex items-center justify-center min-h-screen px-4 w-full py-10">
      <AnimatePresence mode="wait">
        <motion.div 
          key={mode} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
          className="w-full max-w-md z-20"
        >
          <GlassCard className="p-8 border-white/20 shadow-2xl backdrop-blur-3xl bg-black/40">
            <div className="text-center mb-6">
                <div className="w-12 h-12 rounded-full bg-white/10 mx-auto mb-2 flex items-center justify-center"><Lock size={20} /></div>
                <h2 className="font-serif text-2xl">{mode === 'login' && 'Welcome Back'}{mode === 'register' && 'Create Sanctuary'}{mode.startsWith('forgot') && 'Recovery'}</h2>
            </div>
            ...
            {mode === 'register' && (
                <form onSubmit={handleRegister} className="space-y-3 max-h-[60vh] overflow-y-auto scrollbar-hide">
                    ...
                    <input type="text" placeholder="Username" value={regUsername} onChange={e => setRegUsername(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white" required />
