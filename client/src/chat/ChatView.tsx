...
import { 
  ...
  Mic, MicOff, X, Zap, Leaf, Search, Paperclip, Sparkles, 
...
export const ChatView: React.FC<ChatViewProps> = ({ onMobileMenuClick, onOpenWidget }) => {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event: any) => {
        ...
        if (isDictating) {
            setInput(prev => prev + (prev.length && !prev.endsWith(' ') ? ' ' : '') + currentText);
  ...
  const toggleVoiceMode = () => {
    if (isStandardMode) return alert("Voice Mode requires Premium Credits. Upgrade to Pro.");
  ...
  const toggleDictation = () => {
     ...
     else { try { recognitionRef.current?.start(); setIsDictating(true); } catch(e) { console.error("Dictation start failed", e); } }
  };
  ...
  const scrollToBottom = (instant = false) => {
    ...
    if (container) {
      // Use 'auto' (instant) when streaming to prevent lag behind the chat bar
      container.scrollTo({ 
  ...
  // Scroll trigger: Use instant scroll when AI is typing to keep up with stream
  useEffect(() => {
    const t = setTimeout(() => {
      // If typing (streaming), use instant scroll to prevent lag
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
     if (isStandardMode) { setError("Vision Analysis requires Premium Credits."); setTimeout(() => setError(null), 3000); return; }
  ...
  const copyToClipboard = (text: string) => {
    if (navigator.clipboard?.writeText) navigator.clipboard.writeText(text).catch(() => alert("Copy failed."));
    else alert("Clipboard API unavailable.");
  };
  
  // FIX: Reply Logic
  const handleReply = (contentOrId: string) => {
      let resolved = contentOrId;
      const foundById = messages.find(m => m.id === contentOrId);
      if (foundById) resolved = foundById.content;
      else {
          const maybeMatch = messages.find(m => m.content === contentOrId);
          if (maybeMatch) resolved = maybeMatch.content;
      }

      const cleanContent = resolved.split('\n')[0].trim().slice(0, 100);
      setReplyingTo(cleanContent);
  ...
  const handleSend = async (e?: React.FormEvent, overrideInput?: string) => {
    ...
    const userMsg: ChatMessage = { role: 'user', content: overrideInput || input, timestamp: Date.now() };
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: '', timestamp: Date.now(), id: modelMessageId }]);
        ...
        while (true) {
            ...
            for (const line of lines) {
                if (line.startsWith('data: ')) {
                    ...
                    } catch (e: any) { if (e.message?.includes("Upgrade")) setError(e.message); }
                }
            }
        }
      }
  ...
  const processMagicTags = (text: string) => {
    ...
    if (flashMatch && !showFlash) { setShowFlash(true); setTimeout(() => { setTheme(flashMatch[1]); setTimeout(() => setShowFlash(false), 500); }, 300); }
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center overflow-hidden">
      ...
      <AnimatePresence>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/90 backdrop-blur-3xl flex flex-col items-center justify-center">
               ...
               <div className="relative mb-12">
                 ...
                 <div className="w-48 h-48 rounded-full border border-white/10 bg-black/50 backdrop-blur-2xl relative z-10 flex items-center justify-center"><Headphones size={64} className={isListening ? "text-white" : "text-white/30"} /></div>
              </div>
              ...
              <button onClick={isListening ? stopListening : startListening} className="mt-12 p-6 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-all">{isListening ? <Mic size={32} /> : <MicOff size={32} className="text-red-400" />}</button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <motion.div initial={{ y: -50 }} animate={{ y: 0 }} className="absolute top-0 w-full z-30 pt-6 px-4 pointer-events-none flex justify-center">
         <div className="absolute left-4 top-6 pointer-events-auto md:hidden"><button onClick={onMobileMenuClick} className="p-2.5 rounded-full bg-black/20 backdrop-blur-xl border border-white/10 text-white/70"><Menu size={20} /></button></div>
         ...
         <div className="absolute right-4 top-6 pointer-events-auto flex items-center gap-3">
             <div className={`px-3 py-1.5 rounded-full backdrop-blur-xl border flex items-center gap-2 shadow-lg transition-colors ${!isStandardMode ? 'bg-black/30 border-white/10' : 'bg-white/5 border-white/5'}`}>
                ...
                <span className={`text-xs font-mono font-bold ${!isStandardMode ? 'text-white/60' : 'text-gray-400'}`}>{!isStandardMode && localCredits > 100 ? '∞' : `${localCredits} Premium`}</span>
             </div>
             <button onClick={toggleVoiceMode} className="w-10 h-10 rounded-full bg-black/30 border border-white/10 backdrop-blur-xl flex items-center justify-center text-white/70 hover:text-white hover:bg-white/10 transition-all shadow-lg relative group"><Headphones size={18} /></button>
         </div>
      </motion.div>
      ...
      <div
        ...
        className="flex-1 w-full max-w-4xl overflow-y-auto px-4 md:px-8 pt-28 pb-4 scrollbar-hide flex flex-col"
      >
        <div className="flex flex-col justify-start min-h-0">
           {renderMessages()}
           {/* FIX: Large spacer at the bottom guarantees the last message is visible above the input bar */}
           <div style={{ height: '280px' }} ref={messagesEndRef} />
        </div>
      </div>

      {/* Input Area */}
      <div className="fixed bottom-0 w-full px-4 md:px-0 md:w-[700px] z-30">
        <div className="flex flex-col gap-2">
          ...
          <div className={`relative flex items-center gap-3 bg-[#0a0e17]/80 backdrop-blur-2xl border border-white/10 p-2 pr-2 pl-3 shadow-[0_0_40px_rgba(0,0,0,0.3)] transition-all ${replyingTo ? 'rounded-b-[2rem] rounded-t-none' : 'rounded-[2rem]'}`}>
              <div className="flex items-center gap-1">
                  <button onClick={() => fileInputRef.current?.click()} disabled={isStandardMode} className={`p-2.5 rounded-full transition-all relative ${attachedImage ? 'bg-white/10 text-white' : 'text-white/40 hover:bg-white/5 hover:text-white'} ${isStandardMode ? 'opacity-30 cursor-not-allowed' : ''}`}><ImageIcon size={20} /></button>
                  <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleImageSelect} />
                  <button onClick={toggleDictation} className={`p-2.5 rounded-full transition-all ${isDictating ? 'bg-red-500/20 text-red-400' : 'text-white/40 hover:bg-white/5 hover:text-white'}`}>{isDictating ? <MicOff size={20} /> : <Mic size={20} />}</button>
              </div>
              ...
              <form onSubmit={handleSend} className="flex-1 flex items-center relative h-full">
                  ...
                  <div className="relative">
                      <button type="button" onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`p-2 transition-colors ${showEmojiPicker ? 'text-white' : 'text-white/30 hover:text-white'}`}><Smile size={20} /></button>
                      {showEmojiPicker && (
