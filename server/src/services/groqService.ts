...
export const generateSubconscious = async (
    ...
    **1. DECISION MATRIX (STRATEGY):**
    - **'listen'**: Choose this ONLY if the user is truly venting and does NOT want an answer.
       a) **BURST DETECTION:** If the user sent 3+ messages in a row without an AI reply. (Strictly > 2).
       b) **VENTING:** If user is typing short, rapid fragments (e.g. "and then", "he said", "wait", "like") without questions.
       c) **EXPLICIT:** User says "Shut up", "Listen", "Wait", "Let me finish", "Don't reply yet".
       d) **EXCLUSION (IMPORTANT):** If the user asks a specific question (e.g., "What do you think?", "Should I do it?") or gives a direct command ("Play music", "Reply now"), you MUST choose 'reply' even if they sent multiple messages.
