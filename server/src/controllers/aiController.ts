...
export const generateVibePlaylist = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const { languages, moods, genres, duration, year } = req.body; // Received from frontend (duration in minutes)
        ...
        const songTitles = await getVibePlaylist(recentMessages, languages, moods, duration, year, genres);
