...
@app.get("/")
def health_check():
    return {"status": "online", "gpu": "available" if tts_engine.device == "cuda" else "cpu"}
...
async def speak(
    ...
    - If NOT provided: Uses Kokoro-82M (Default Voice).
    """
    try:
        voice_bytes = None
        if voice_sample:
        ...
        audio_buffer = tts_engine.generate(text, voice_bytes)
